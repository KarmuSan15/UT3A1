...
//&INDEX EXAMEN UT3 RECUPERACION

// Importo los módulos necesarios
const express = require('express');
const cors = require('cors');

// Importo los servicios que gestionan la lógica de login, items, usuarios y devaluación
const login = require('./services/login');
const items = require('./services/items');
const users = require('./services/users');
const devaluacion = require('./services/devaluacion');  // Importa el servicio de devaluación
...
// **NUEVO**: Endpoint para obtener todos los registros de devaluación desde la base de datos
app.get('/getDevaluacion', async (req, res) => {
  try {
    await devaluacion.getDevaluacion(req, res);  // Llama a la función getDevaluacion del archivo devaluacion.js
  } catch (err) {
    console.error(`Error while getting devaluacion: ${err.message}`);
    res.status(500).json({ message: 'Error al obtener los datos de devaluación' });
  }
});

// **NUEVO**: Endpoint para agregar un nuevo registro de devaluación
app.post('/addDevaluacion', async (req, res) => {
  try {
    await devaluacion.addDevaluacion(req, res);  // Llama a la función addDevaluacion del archivo devaluacion.js
  } catch (err) {
    console.error(`Error while adding devaluacion: ${err.message}`);
    res.status(500).json({ message: 'Error al agregar la devaluación' });
  }
});
